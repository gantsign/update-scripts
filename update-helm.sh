...
new_version="$(curl --silent -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/helm/helm/releases \
...
commit_msg="\
Updated Helm to $new_version

Keeping up with the latest changes.
"

branch_name="helm-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/helm_version: '.*'/helm_version: '$new_version'/" README.md

    sed --in-place \
        "s/helm_version: '.*'/helm_version: '$new_version'/" defaults/main.yml
}
