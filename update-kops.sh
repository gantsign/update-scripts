...
commit_msg="\
Updated kops to $new_version

Keeping up with the latest changes.
"

branch_name="kpos-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/kops_version: '.*'/kops_version: '$new_version'/" README.md

    sed --in-place \
        "s/kops_version: '.*'/kops_version: '$new_version'/" defaults/main.yml
}
