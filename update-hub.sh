...
new_version="$(curl --silent -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/github/hub/releases \
...
new_sha256sum="$(curl --silent --location --output - \
    "https://github.com/github/hub/releases/download/v${new_version}/hub-linux-amd64-${new_version}.tgz" \
...
commit_msg="\
Updated Hub to $new_version

Keeping up with the latest changes.
"

branch_name="hub-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/hub_version: '.*'/hub_version: '$new_version'/" README.md

    sed --in-place \
        "s/hub_version: '.*'/hub_version: '$new_version'/" defaults/main.yml

    sed --in-place \
        "s/hub_redis_sha256sum: '.*'/hub_redis_sha256sum: '$new_sha256sum'/" README.md

    sed --in-place \
        "s/hub_redis_sha256sum: '.*'/hub_redis_sha256sum: '$new_sha256sum'/" defaults/main.yml
}
