...
new_version="$(curl --silent -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/kubernetes/minikube/releases \
...
commit_msg="\
Updated minikube to $new_version

Keeping up with the latest changes.
"

branch_name="minikube-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/minikube_version: '.*'/minikube_version: '$new_version'/" README.md

    sed --in-place \
        "s/minikube_version: '.*'/minikube_version: '$new_version'/" defaults/main.yml
}
