...
new_version="$(curl --silent -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/sharkdp/bat/releases \
...
new_sha256sum="$(curl --silent --location --output - \
    "https://github.com/sharkdp/bat/releases/download/v${new_version}/bat_${new_version}_amd64.deb" \
...
commit_msg="\
Updated bat to $new_version

Keeping up with the latest changes.
"

branch_name="bat-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/bat_version: '.*'/bat_version: '$new_version'/" README.md

    sed --in-place \
        "s/bat_version: '.*'/bat_version: '$new_version'/" defaults/main.yml

    sed --in-place \
        "s/bat_redis_sha256sum: '.*'/bat_redis_sha256sum: '$new_sha256sum'/" README.md

    sed --in-place \
        "s/bat_redis_sha256sum: '.*'/bat_redis_sha256sum: '$new_sha256sum'/" defaults/main.yml
}
