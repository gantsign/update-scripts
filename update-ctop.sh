...
new_version="$(curl --silent -H 'Accept: application/vnd.github.v3+json' https://api.github.com/repos/bcicen/ctop/releases \
...
new_sha256sum="$(curl --silent --location --output - \
    "https://github.com/bcicen/ctop/releases/download/v${new_version}/sha256sums.txt" \
    | grep --fixed-strings --color=never "ctop-${new_version}-linux-amd64" \
    | awk '{print $1}')"
...
commit_msg="\
Updated ctop to $new_version

Keeping up with the latest changes.
"

branch_name="ctop-$new_version"

changed_file='README.md'

update_files() {
    sed --in-place \
        "s/ctop_version: '.*'/ctop_version: '$new_version'/" README.md

    sed --in-place \
        "s/ctop_version: '.*'/ctop_version: '$new_version'/" defaults/main.yml

    sed --in-place \
        "s/ctop_redis_sha256sum: '.*'/ctop_redis_sha256sum: '$new_sha256sum'/" README.md

    sed --in-place \
        "s/ctop_redis_sha256sum: '.*'/ctop_redis_sha256sum: '$new_sha256sum'/" defaults/main.yml
}
